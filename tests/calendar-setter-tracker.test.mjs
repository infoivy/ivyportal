...
const migrationPath = "supabase/migrations/20260729102800_setter_tracking_sheet.sql";
...
test("Calendar is a refined three-view workspace with explicit failure handling", () => {
  ...
  assert.match(calendar, /max-w-none/);
  assert.match(calendar, /calView === "month"/);
...
test("setter tracking migration adds lifecycle fields and durable follow-up history", () => {
  ...
  assert.match(
    migration,
    /set_id uuid not null references public\.set_reminders\(id\) on delete restrict/i,
  );
  ...
  assert.match(
    migration,
    /revoke all on table public\.set_follow_ups from public, anon, authenticated/i,
  );
  assert.match(
    migration,
    /grant select, insert, update on table public\.set_follow_ups to authenticated/i,
  );
  assert.doesNotMatch(migration, /grant[^;]*delete[^;]*set_follow_ups/i);
  assert.match(
    migration,
    /revoke all on table public\.set_reminder_events from public, anon, authenticated/i,
  );
  ...
  assert.match(
    migration,
    /grant select, insert, update on table public\.set_reminders to authenticated/i,
  );
...
test("database and server authorization isolate setter sheets while preserving leadership oversight", () => {
  ...
  assert.match(calendarFunctions, /targetCanOwnSets/);
  assert.match(calendarFunctions, /Sets can only be assigned to an active real sales profile/);

...
- Co-founders who close earn a FLAT 10% — even on set+close — capped at $1,000 per Mon–Sun week and $2,000 per calendar month, paid across the two semi-monthly payouts (founder-confirmed 2026-07-12; implemented in `src/lib/payouts-calc.ts`).
...
- Student flow (founder-directed 2026-07-18, amended 2026-07-25): a new student first confirms their own timezone (soft lock; editable later on Profile), then the portal is LOCKED to the five Start Here steps (onboarding form → join offer board → walkthrough Loom (embedded) → Skool training videos → offer board course; identical for both pathways) until all are done — completion auto-stamps `students.onboarding_completed_at`, advances phase to coaching, notifies fulfillment (bell + team chat), and auto-assigns 1:1 students the "Book your 1:1 coaching calls" task. After unlock, Start Here is REMOVED entirely. Students see NO phase journey/pipeline language and NO Placements tab (placements are staff-managed). Daily student targets: ALWAYS 3 roleplays; pre-approval 3 looms/day into the Inner Circle Loom Review chat (NOT to offers); after a CSM approves (phase → applying) 5 loom applications/day — the daily EOD shows exactly one loom field per stage, no Replies, no Summary. Weekly EOD sits UNDER the daily one, records WHICH group calls were attended (names from `org_settings.group_call_schedule`, admin-editable, no times) plus self-reported 1:1 calls (they only get `calls_allotted`, usually 10), turns loud from Sunday, and AUTO-SUBMITS from live attendance ticks at ~23:45 Sunday in the student's own timezone (pg_cron `student-weekly-eod-autosubmit`). Students can never DELETE EODs (daily or weekly) — only adjust; deletion is admin/coach/CSM only. Program type derives from `calls_allotted > 0` (1:1) vs 0 (group). Graduated phases (offer_won/testimonial/graduated) owe no EODs and see the graduation page only.

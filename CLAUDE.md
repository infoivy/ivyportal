...
UI checks use `const { roles } = useAuth()` and `roles.includes('role')`. Do not treat a hidden button or route guard as security. New tables and storage objects must have RLS enabled with policies that enforce the same server-side rule. Gathering Hub, Instagram/content planning, and founder-only docs are founder-only; founder_settings (operational: cash goal, CRM toggle, quarterly goals) remain admin-manageable. Payment details, revenue, and installment records are closer/admin-only unless a narrower existing policy says otherwise. Whop is the source of truth for cash-in (founder-stated 2026-07-12): the Finance page reads Whop and reconciles it against logged deals/paid installments (amount ±$1 within ±3 days = same money, never summed twice); unmatched items on either side surface as the gap. Setters do not have Sales or Revenue access (founder-confirmed 2026-07-11); their defaults are Dashboard, EODs, Action Items, Training, Calendar, Testimonials, Knowledge, Notes. Service credentials are admin-only.

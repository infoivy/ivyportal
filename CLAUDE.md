...
- Student flow (founder-directed 2026-07-18): a new student's portal is LOCKED to the five Start Here steps (onboarding form → join offer board → walkthrough Loom → Skool training videos → offer board course; identical for both pathways) until all are done — completion auto-stamps `students.onboarding_completed_at`, advances phase to coaching, notifies fulfillment (bell + team chat), and auto-assigns 1:1 students the "Book your 1:1 coaching calls" task. Daily student targets: ALWAYS 3 roleplays; pre-approval 3 looms/day into the Inner Circle Loom Review chat (NOT to offers); after a CSM approves (phase → applying) 5 loom applications/day — the daily EOD shows exactly one loom field per stage, no Replies, no Summary. Weekly EOD sits UNDER the daily one, records WHICH group calls were attended (names from `org_settings.group_call_schedule`, admin-editable, no times) plus self-reported 1:1 calls (they only get `calls_allotted`, usually 10), and only turns loud from Sunday or when overdue. Program type derives from `calls_allotted > 0` (1:1) vs 0 (group).

    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            destinationContent
                ...
                .environment(\.openPortalMenu) { withAnimation(ivySpring) { menuPresented = true } }
                .transition(.opacity)
                .id(surface)
            floatingTabBar
            // Mochi-style slide-over sidebar (full height, left panel)
            if menuPresented {
                ...
                HStack(spacing: 0) {
                    PortalMenuSheet(
                        ...
                        close: { withAnimation(ivySpring) { menuPresented = false } }
                    )
                    .frame(width: 300)
                    Spacer(minLength: 0)
                }

...
export function StatCard({ label, value, icon, accent, hint, delta, sparkData, prevSparkData, noData }: StatCardProps) {
  return (
    <div className="relative card-surface p-5 overflow-hidden">
      ...
      {noData ? (
        <div className="text-caption text-muted-foreground bg-muted rounded-full px-3 py-1 w-fit">
          No data
        </div>
      ) : (
        <div className="flex items-baseline gap-2 flex-wrap">
          <div className="text-metric text-foreground num">{value}</div>
          {delta !== undefined && <DeltaChip {...delta} />}
        </div>
      )}
      {hint && <div className="text-caption text-muted-foreground mt-1.5">{hint}</div>}

...
import { ListTodo, PhoneCall, Plus, ArrowRight } from "lucide-react";
...
export function CsmTodayQueue() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const [students, tallies, notes, calls] = await Promise.all([
        ...
        supabase.from("csm_tally").select("student_id, created_at").gte("created_at", fourteen),
      ...
      for (const t of (tallies.data ?? []) as any[]) bump(t.student_id, t.created_at);
      ...
      return { students: (students.data ?? []) as { id: string; full_name: string; phase: string }[], lastTouch };
    },
  });
  ...
  const checkIn = async (studentId: string) => {
    ...
    qc.invalidateQueries({ queryKey: ["csm-today-queue"] });
  };
            ...
            return (
              <div key={s.id} className="flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-md px-2.5 py-2 hover:bg-muted/50 motion-safe:transition-colors">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Popover open={noteFor === s.id} onOpenChange={(o) => setNoteFor(o ? s.id : null)}>

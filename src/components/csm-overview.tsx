...
export function CsmOverview() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const [studentsRes, genericTallyRes, studentTallyRes, callsRes, eodsRes, placementsRes] = await Promise.all([
        supabase.from("students").select("id, phase, status, offer_landed_at, first_win_at").eq("is_demo", false).is("archived_at" as never, null),
        supabase.from("csm_tally").select("kind, created_at").is("student_id", null).gte("created_at", eightWeeksAgo),
        supabase.from("csm_tally").select("kind, created_at, students!inner(is_demo)").eq("students.is_demo", false).is("students.archived_at" as never, null).gte("created_at", eightWeeksAgo),
        supabase.from("student_calls").select("id, call_date, status, students!inner(is_demo)").eq("students.is_demo", false).is("students.archived_at" as never, null).is("voided_at", null).gte("call_date", iso(subDays(new Date(), 8 * 7))),
        ...
        supabase.from("student_placements").select("stage, students!inner(is_demo)").eq("students.is_demo", false).is("students.archived_at" as never, null).is("voided_at", null),
      ]);
      return {
        students: studentsRes.data ?? [],
        tally: [...(genericTallyRes.data ?? []), ...(studentTallyRes.data ?? [])],
        calls: callsRes.data ?? [],
        eods: eodsRes.data ?? [],

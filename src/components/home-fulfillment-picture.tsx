...
import {
  ...
  HeartPulse, MessageCircle, Users, ArrowRight,
} from "lucide-react";
...
/**
 * Faizan's home picture (founder 2026-07-31): he runs fulfillment, so his
 * overview leads with delivery truth — are students active, checked in,
 * reporting, and being worked by the CSMs. Same tile language as the
 * default leadership brief; the depth is in the CSM activity table.
 */
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const [studentsRes, checkinsRes, tallyGenRes, tallyStuRes, eodsRes, itemsRes, callsRes, csmRolesRes, profilesRes] = await Promise.all([
        supabase.from("students").select("id, phase, status").eq("is_demo", false),
      ...
      const students = (studentsRes.data ?? []) as { id: string; phase: string; status: string }[];
      ...
      // from csm_tally (generic + student-scoped merged).
  ...
  return (
    <section className="card-surface overflow-hidden" aria-labelledby="fulfillment-picture-title">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3">
        <Tile icon={Users} label="Active students" detail="Current delivery roster" value={d?.activeCount} to="/students" />

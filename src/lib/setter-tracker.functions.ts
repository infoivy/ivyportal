...
async function requireSalesTrackerAccess(context: TrackerContext) {
  ...
  const canViewTeam = roles.some((role: SalesRole) =>
    TEAM_TRACKER_ROLES.includes(role as (typeof TEAM_TRACKER_ROLES)[number]),
  );
  return { roles, canViewTeam };
}

async function assertTrackerTarget(context: TrackerContext, targetUserId: string) {
...
export const listSetterTrackerMembers = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({ context }): Promise<{ canViewTeam: boolean; members: SetterTrackerMember[] }> => {
      const access = await requireSalesTrackerAccess(context);
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const getSetterTracker = createServerFn({ method: "GET" })
  ...
  .validator((input: { targetUserId?: string; days?: number } | undefined) => {
    ...
    const targetUserId = input?.targetUserId
      ? requireUuid(input.targetUserId, "setter id")
      : undefined;
  ...
  .handler(async ({ data, context }): Promise<SetterTrackerData> => {
    ...
    const [setsResult, eodsResult] = await Promise.all([
      context.supabase
        .from("set_reminders")
        .select(
          "id, prospect, event_start, duration_min, owner_id, source, status, confirmed_at, reminder_log, notes, lead_channel, qualification_status, attendance_status, sales_outcome, outcome_recorded_at, calendar_sync_status, calendar_sync_error, updated_at",
        )
      ...
      context.supabase
        ...
        .select(
          "id, user_id, report_date, dials, leads_contacted, dms_sent, convos_started, calls_booked, calls_scheduled, shows, no_shows, closes, created_at",
        )
    ...
    if (sets.length > 0) {
      ...
      const [followUpsResult, eventsResult] = await Promise.all([
        context.supabase
          .from("set_follow_ups")
          .select(
            "id, set_id, due_at, channel, status, note, completed_at, completed_by, created_by, created_at, updated_at",
          )
          .in("set_id", setIds)
...
export const updateSetLifecycle = createServerFn({ method: "POST" })
  ...
  .validator(
    (input: {
      ...
      const leadChannel =
        input.leadChannel === undefined
          ? undefined
          : includesValue(LEAD_CHANNELS, input.leadChannel)
            ? input.leadChannel
            : (() => {
                throw new Error("Invalid lead channel");
              })();
      const qualificationStatus =
        input.qualificationStatus === undefined
          ? undefined
          : includesValue(QUALIFICATION_STATUSES, input.qualificationStatus)
      ...
      const attendanceStatus =
        input.attendanceStatus === undefined
          ? undefined
          : includesValue(ATTENDANCE_STATUSES, input.attendanceStatus)
            ? input.attendanceStatus
            : (() => {
                throw new Error("Invalid attendance status");
              })();
      const salesOutcome =
        input.salesOutcome === undefined
          ? undefined
          : includesValue(SALES_OUTCOMES, input.salesOutcome)
            ? input.salesOutcome
            : (() => {
                throw new Error("Invalid sales outcome");
              })();
      const notes = optionalNote(input.notes);
      if (
        [leadChannel, qualificationStatus, attendanceStatus, salesOutcome, notes].every(
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.qualificationStatus !== undefined)
      patch.qualification_status = data.qualificationStatus;
    ...
    const { data: row, error } = await context.supabase
      ...
      .eq("id", data.id)
      .select(
        "id, lead_channel, qualification_status, attendance_status, sales_outcome, outcome_recorded_at, notes, updated_at",
      )
      .single();
    if (error) throw new Error(error.message);
    return row;
  ...
  .validator((input: { setId: string; dueAt: string; channel?: string; note?: string | null }) => {
    ...
    const channel =
      input.channel === undefined
        ? "dm"
        : includesValue(FOLLOW_UP_CHANNELS, input.channel)
          ? input.channel
          : (() => {
  ...
  .handler(async ({ data, context }) => {
    ...
    const query = existing
      ? context.supabase
          ...
          .eq("id", existing.id)
      : context.supabase.from("set_follow_ups").insert({
          set_id: data.setId,
          due_at: data.dueAt,
  ...
  .validator((input: { id: string; status?: string }) => {
    ...
    const status =
      input.status === undefined
        ? "completed"
        : includesValue(FOLLOW_UP_STATUSES, input.status)
          ? input.status
          : (() => {

  ...
  return (
    <div className="min-h-full bg-background">
      <PageShell>
        ...
        <Card className="card-surface border-0 p-4 shadow-none">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex flex-wrap items-center gap-1">
              ...
              <Button className="h-10 w-10" size="icon" variant="ghost" onClick={() => setWeekStart((w) => (daySpan === 7 ? subWeeks(w, 1) : addDays(w, -daySpan)))} aria-label="Previous dates">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Button className="min-h-10" size="sm" variant="ghost" onClick={() => { const now = shiftToTz(new Date(), tz); now.setHours(0, 0, 0, 0); setWeekStart(daySpan === 7 ? startOfWeek(now, { weekStartsOn: 1 }) : now); }}>
                Today
              </Button>
              <Button className="h-10 w-10" size="icon" variant="ghost" onClick={() => setWeekStart((w) => (daySpan === 7 ? addWeeks(w, 1) : addDays(w, daySpan)))} aria-label="Next dates">
        ...
        <Card className="card-surface relative overflow-hidden border-0 p-0 shadow-none">
            ...
            {days.map((d) => {
              ...
              return (
                <div key={d.toISOString()} className="sticky top-0 z-20 border-b border-l border-border/60 bg-card px-2 py-2 text-center">
                  <div className="text-micro uppercase tracking-wide text-muted-foreground">{format(d, "EEE")}</div>
                  <div className={`mx-auto mt-1 grid h-7 w-7 place-items-center rounded-full text-body font-semibold tabular-nums ${today ? "bg-foreground text-background" : "text-foreground"}`}>{format(d, "d")}</div>
                </div>
              );
            })}

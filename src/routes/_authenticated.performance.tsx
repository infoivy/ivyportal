...
function buildDayList(days: number) {
  return Array.from({ length: days }, (_, index) => format(subDays(new Date(), days - index - 1), "yyyy-MM-dd"));
}
...
function PerformancePage() {
  ...
  const performanceQ = useQuery({
    ...
    queryFn: async (): Promise<PerformanceData> => {
      ...
      const [activityRes, profileRes, roleRes] = await Promise.all([
        ...
        supabase.from("profiles").select("id, display_name, active, created_at").eq("is_demo", false),
        supabase.from("user_roles").select("user_id, role").in("role", [...REPORTING_ROLES, "founder", "cofounder", "admin"]),
      ]);

      if (activityRes.error) throw new Error(`Performance activity failed: ${activityRes.error.message}`);
      if (profileRes.error) throw new Error(`Performance profiles failed: ${profileRes.error.message}`);
      ...
      const roster = ids
        .map((id): RosterMember => {
          ...
          return {
            ...
            role: ROLE_PRIORITY.find((role) => memberRoles.includes(role)) ?? memberRoles[0] ?? "member",
  ...
  const trend = useMemo(() => {
    ...
    return dayList.map((date) => ({
      ...
      value: sum(selectedRows.filter((row) => row.report_date === date), metric),
    }));
  ...
  const accountability = useMemo(() => {
    ...
    return data.roster.map((member) => {
      ...
      const earlierBooked = sum(rows.filter((row) => earlierDays.has(row.report_date)), "calls_booked");
      const recentBooked = sum(rows.filter((row) => recentDays.has(row.report_date)), "calls_booked");
      const trendDirection = earlierBooked == null && recentBooked == null
        ? "Unavailable"
      ...
      return {
        ...
        coverage: availableDays.length ? Math.round((submittedDays / availableDays.length) * 100) : null,
    ...
    return (
      <PageShell className="pb-24 sm:pb-6">
        <div className="card-surface mx-auto mt-14 max-w-xl p-7 text-center">
          ...
          <h1 className="mt-4 text-title font-semibold text-foreground">Performance could not load</h1>
          <p className="mt-2 text-body text-muted-foreground">
            {performanceQ.error instanceof Error ? performanceQ.error.message : "The reporting source is temporarily unavailable."}
          </p>
  ...
  const showDenominator = totalShows != null && totalNoShows != null ? totalShows + totalNoShows : null;
  const showRate = showDenominator ? Math.round(((totalShows ?? 0) / showDenominator) * 100) : null;
  const closeRate = totalShows && totalCloses != null ? Math.round((totalCloses / totalShows) * 100) : null;
  ...
  const memberOptions = [
    ...
    ...(user?.id && data.roster.some((member) => member.id === user.id) ? [{ value: user.id, label: "My activity" }] : []),
    ...data.roster.filter((member) => member.id !== user?.id).map((member) => ({ value: member.id, label: member.name })),
  ];
  const selectedMemberLabel = memberOptions.find((option) => option.value === memberId)?.label ?? "All team";
  ...
  return (
    <PageShell wide className="pb-24 sm:pb-7">
      <PageHeader
        ...
        actions={canSubmitEod ? (
          <Button asChild>
            <Link to="/eods"><FileText className="h-4 w-4" /> Open EOD</Link>
          </Button>
        ) : null}
      />
        ...
        <Sheet>
          <SheetTrigger asChild>
            ...
            >
              ...
              <span className="truncate text-caption font-normal text-muted-foreground">{days}D · {metricLabel} · {selectedMemberLabel}</span>
            </button>
          </SheetTrigger>
          <SheetContent side="bottom" className="rounded-t-2xl pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
            <SheetHeader className="text-left">
              ...
              <SheetDescription>Change the reporting window, activity metric, or team member.</SheetDescription>
            </SheetHeader>
            <div className="mt-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <SelectField className="h-12 text-body" value={metric} onChange={(value) => setMetric(value as MetricKey)} options={METRICS} />
              </div>
              <div className="space-y-2">
                <div className="text-caption font-semibold text-foreground">Team member</div>
                <SelectField className="h-12 text-body" value={memberId} onChange={setMemberId} options={memberOptions} />
              </div>
            </div>
          </SheetContent>
        </Sheet>
      ...
      <section className="card-surface p-5 sm:p-6" aria-labelledby="performance-summary-title">
        <div className="grid gap-6 md:grid-cols-[minmax(220px,0.7fr)_minmax(0,1.3fr)] md:items-center">
          <div>
            <p className="text-micro font-semibold uppercase tracking-[0.1em] text-muted-foreground">Reporting window</p>
            <h2 id="performance-summary-title" className="mt-1 text-title font-semibold text-foreground">Performance summary</h2>
            <div className="mt-5 text-[44px] font-semibold leading-none tracking-[-0.04em] tabular-nums text-foreground">{formatMetric(totalBooked)}</div>
            <p className="mt-2 text-caption text-muted-foreground">calls booked from submitted EODs</p>
          </div>
          <dl className="divide-y divide-border border-y border-border">
            <SummaryRow
              label="Show rate"
              value={showRate == null ? "Unavailable" : `${showRate}%`}
              detail={showRate == null ? "No verified show/no-show denominator" : `${formatMetric(totalShows)} shows · ${formatMetric(totalNoShows)} no-shows`}
            />
            <SummaryRow
              label="Close rate"
              value={closeRate == null ? "Unavailable" : `${closeRate}%`}
              detail={closeRate == null ? "No verified shows denominator" : `${formatMetric(totalCloses)} closes from ${formatMetric(totalShows)} shows`}
            />
            <SummaryRow
              label="EOD coverage"
              value={coverage == null ? "Unavailable" : `${coverage}%`}
              detail={coverage == null ? "No expected reporting days in this range" : `${submitted} submitted · ${Math.max(0, expected - submitted)} missing`}
            />
          </dl>
        </div>
      </section>
      ...
      <section className="card-surface p-4 sm:p-6" aria-labelledby="activity-trend-title">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-micro font-semibold uppercase tracking-[0.1em] text-muted-foreground">One canonical graph</p>
            <h2 id="activity-trend-title" className="mt-1 text-title font-semibold text-foreground">Activity trend</h2>
            <p className="mt-1 text-caption text-muted-foreground">{metricLabel} by reporting date.</p>
          </div>
          <div className="hidden w-[210px] sm:block">
            <SelectField value={metric} onChange={(value) => setMetric(value as MetricKey)} options={METRICS} />
          </div>
        </div>
        <div className="mt-5">
          {trend.some((point) => point.value != null) ? (
            <VolumeAreaChart
              data={trend}
              xKey="label"
              series={[{ key: "value", label: metricLabel, color: "var(--color-foreground)", strokeWidth: 2 }]}
              height={280}
              showDots
            />
          ) : (
      ...
      <section className="card-surface overflow-hidden" aria-labelledby="team-accountability-title">
        <div className="border-b border-border px-4 py-4 sm:px-5">
          <p className="text-micro font-semibold uppercase tracking-[0.1em] text-muted-foreground">Submission and outcome record</p>
          <h2 id="team-accountability-title" className="mt-1 text-title font-semibold text-foreground">Team accountability</h2>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {accountability.map((member) => (
                <tr key={member.id} className="hover:bg-muted/40 motion-safe:transition-colors">
                  <td className="px-5 py-3.5 text-body font-semibold text-foreground">{member.name}</td>
                  <td className="px-4 py-3.5 text-caption capitalize text-muted-foreground">{member.role}</td>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      <span className={`text-body font-semibold tabular-nums ${member.coverage != null && member.coverage < 70 ? "text-destructive" : "text-foreground"}`}>{member.coverage == null ? "Unavailable" : `${member.coverage}%`}</span>
                      <span className="text-caption text-muted-foreground">{member.submittedDays}/{member.expectedDays}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-right text-body tabular-nums text-foreground">{formatMetric(member.booked)}</td>
                  <td className="px-4 py-3.5 text-right text-body tabular-nums text-foreground">{formatMetric(member.shows)}</td>
                  <td className="px-4 py-3.5 text-right text-body tabular-nums text-foreground">{formatMetric(member.closes)}</td>
                  <td className="px-5 py-3.5 text-right text-caption text-muted-foreground">{member.trendDirection}</td>
                </tr>
              ))}
              {!accountability.length && (
                <tr><td colSpan={7} className="px-5 py-10 text-center text-caption text-muted-foreground">No reporting team members were found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <article key={member.id} className="px-4 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-0.5 text-caption capitalize text-muted-foreground">{member.role}</p>
                </div>
                <span className={`text-body font-semibold tabular-nums ${member.coverage != null && member.coverage < 70 ? "text-destructive" : "text-foreground"}`}>{member.coverage == null ? "Unavailable" : `${member.coverage}% EOD`}</span>
              </div>
      ...
      <div className="flex flex-col gap-2 px-1 text-micro text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        <span>Source: real-only submitted `eods_activity_real` records. Demo, Revenue, and CRM data are not mixed into this workspace.</span>
        <span>Updated {format(new Date(data.fetchedAt), "h:mm a")}</span>
      </div>

      {canSubmitEod && (
        <Link to="/eods" className="inline-flex min-h-12 items-center gap-2 text-body font-semibold text-foreground hover:opacity-70">
...
function MobileMetric({ label, value }: { label: string; value: number | null }) {
  return (
    <div>
      ...
      <dd className="mt-1 text-body font-semibold tabular-nums text-foreground">{formatMetric(value)}</dd>
    </div>
  );
}
